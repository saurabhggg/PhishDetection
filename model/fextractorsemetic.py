...
# CONFIG
# ------------------------------
CACHE_FILE = "feature_cache_new_v2.json"
...
def build_dataset(input_file, output_csv):
    cache = {}
    ...
    inputVal = pd.read_csv(input_file)
    domains = inputVal['Domain'].astype(str).tolist()
    print(f"📥 Loaded {len(domains)} domains from {input_file}")
    for url in tqdm(domains, desc="Extracting features"):
        if url in cache:
            feats = cache[url]
        else:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--input", default="prefilter_for_heavy.csv", help="Path to feature for heavy")
    ap.add_argument("--output", default="feature_from_heavy_sementic.csv", help="Output CSV file path")
    args = ap.parse_args()
